...
// 这里的nh由fast planner node传入, 设置话题通信，回调函数等
void KinoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  /*  fsm param  */
  // 从参数服务器中提取指定name的参数，若不存在则赋值为default value
...
void KinoReplanFSM::execFSMCallback(const ros::TimerEvent& e) {
  ...
  switch (exec_state_) {
    ...
    case GEN_NEW_TRAJ: {
      ...
      bool success = callKinodynamicReplan(); // 如果成功获取一条轨迹，就改状态为EXEC_TRAJ，去执行
      if (success) {
        changeFSMExecState(EXEC_TRAJ, "FSM");
      } else {
        // have_target_ = false;
        // changeFSMExecState(WAIT_TARGET, "FSM");
        changeFSMExecState(GEN_NEW_TRAJ, "FSM");  // 如果没有则继续保持
      }
      break;
    }
...
bool KinoReplanFSM::callKinodynamicReplan() {
  ...
  if (plan_success) {
    ...
    /* publish traj */
    plan_manage::Bspline bspline;   // bspline.msg 数据
    bspline.order      = 3;
    bspline.start_time = info->start_time_;
    bspline.traj_id    = info->traj_id_;

    Eigen::MatrixXd pos_pts = info->position_traj_.getControlPoint();  // 获得bspline的控制点
